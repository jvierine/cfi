...
cf_names=["$G_{uuu}$","$G_{vvv}$","$G_{www}$"]
sf_names=["uuu","vvv","www"]
# end constants
...
def vel(t,alt,lats,lons,times,rgs,v,dt,dh):
    ...
    '''
    ti=n.array(n.round((t-times[0])/dt),dtype=int)
    hi=n.array(n.floor((alt-rgs[0])/dh),dtype=int)
...
#        print("%d/%d s_z %1.2f h0 %1.2f h1 %1.2f tau %1.2f"%(i,n_times,s_z,n.mean(heights[idx0]),n.mean(heights[idx1]),tau))
    ...
    for pi in range(n_meas):
        ...
        if LTz:
            ...
            A[pi,0] = w*(bragg_k_L**2)*bragg_l_L
            Ao[pi,0] = (bragg_k_L**2)*bragg_l_L
            
            A[pi,1] = w*(bragg_k_T**2.0)*bragg_l_T # kv1*kv2
            Ao[pi,1]= (bragg_k_T**2)*bragg_l_T # kv1*kv2
            
            A[pi,2] = w*(bragg_k_z**2)*bragg_l_z # kw1*kw2
            Ao[pi,2] = (bragg_k_z**2)*bragg_l_z# kw1*kw2
        
            m[pi]=w*((2*n.pi)**3.0)*(dops[k]**2)*dops[l]
            mo[pi]=((2*n.pi)**3.0)*(dops[k]**2)*dops[l] 
            
        else:
            A[pi,0]=w*(braggs[k,0]**2)*braggs[l,0] # ku1*ku2
            Ao[pi,0]=(braggs[k,0]**2)*braggs[l,0] # ku1*ku2
                
            A[pi,1]=w*(braggs[k,1]**2)*braggs[l,1] # kv1*kv2
            Ao[pi,1]=(braggs[k,1]**2)*braggs[l,1] # kv1*kv2
                
            A[pi,2]=w*(braggs[k,2]**2)*braggs[l,2] # kw1*kw2
            Ao[pi,2]=(braggs[k,2]**2)*braggs[l,2] # kw1*kw2
    ...
    try:
        ...
        if False:
            ...
            plt.axhline(-resid_std*5.0,color="red")
            plt.axhline(resid_std*10.0,color="red")
            plt.axhline(-resid_std*10.0,color="red")
            plt.show()

        #
#        plt.plot(resid_std-mean_err,".")
 #       plt.title("residuals")
  #      plt.show()
        
        # remove extreme outliers 
        good_idx=n.where(n.abs(resid_std-mean_err) < 100.0*resid_std)[0]
...
def hor_acfs(meas,h0=90,dh=2,tau=0.0,s_h=n.arange(0,400.0,25.0),
    ...
    '''
    n_lags=len(s_h)
    acfs=n.zeros([n_lags,3])
    errs=n.zeros([n_lags,3])
